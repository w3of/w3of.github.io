...
if __name__ == '__main__':
    createKey()
    with open('WAP-排名结果.txt', 'a', encoding='utf-8') as f:
        f.write('***********'+time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())+'***********'+'\n')
    for k in key_word:
        t = 0
        key_url = parse.quote(k.strip())
        for p in range(0,21,10): # 第二个参数设置页数（前二页11，前三页21，前四页31，以此类推。）
            url = 'https://m.baidu.com/s?wd=%s&pn=%d&rn=10' % (key_url,p)
            html = download(url, user_agent=ua)
            a = re.findall(r"'order':'(\d+)','mu':'(.*?)'", html, re.S | re.I)
            for i in a:
                if 'yhfk.01ny.cn' in i[1]:  # 这里指定需要检测的网址
                    with open('WAP-排名结果.txt', 'a', encoding='utf-8') as f:
